...
def rigg(*args):
    ...
    if len(selection)==0:
        ...
        for n in range(len(all_joint)-1):
            ...
            if len(selection) == 1:
                newcontrol = cmds.circle(name=all_joints[n]+"_ctrl",radius = control_scale)[0] 
            ...
            joint_position = cmds.xform(all_joints[n], query=True, translation=True, worldSpace=True)
            cmds.xform(grp, translation=joint_position)
            orient_cons = cmds.orientConstraint(all_joints[n], grp)
            cmds.delete(orient_cons)
            
            cmds.parentConstraint(newcontrol,all_joints[n])
